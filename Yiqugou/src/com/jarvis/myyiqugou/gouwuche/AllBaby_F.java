package com.jarvis.myyiqugou.gouwuche;

import com.example.adapter.Adapter_ListView_cart;
import com.example.adapter.Adapter_ListView_cart.onCheckedChanged;
import com.example.yiqugou.R;
import com.myyiqugou.data.Data;
import com.yiqugou.myhome.BabyActivity;
import com.yiqugou.mytools.IBtnCallListener;
...
public class AllBaby_F extends Fragment implements IBtnCallListener,
		onCheckedChanged, OnClickListener {
	...
	private void initView(View view) {
			...
			public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
				...
				if (arg1) {
					...
					for (int i = 0; i < Data.arrayList_cart.size(); i++) {
						...
						((CheckBox) (listView_cart.getChildAt(i))
					...
					for (int i = 0; i < Data.arrayList_cart.size(); i++) {
						...
						if (((CheckBox) (listView_cart.getChildAt(i))
								.findViewById(R.id.cb_choice)).isChecked()) {
							// 计算出列表选中状态的数量
							isChoice_all += 1;
						}
					}
		...
		if (Data.arrayList_cart != null && Data.arrayList_cart.size() != 0) {
			adapter = new Adapter_ListView_cart(getActivity(),
					Data.arrayList_cart);
	...
	public void getChoiceData(int position, boolean isChoice) {
		...
		if (isChoice) {
			if (Data.arrayList_cart.size() != 0) {
				...
				Data.Allprice_cart += Float.valueOf(Data.arrayList_cart
						.get(position).get("num").toString()) * 49;
			}
		} else {
			if (Data.arrayList_cart.size() != 0) {
				Data.Allprice_cart -= Float.valueOf(Data.arrayList_cart
	...
	public void onClick(View view) {
		...
		case R.id.tv_cart_buy_or_del:// 点击结算/删除
			...
			if (tv_cart_buy_Ordel.getText().toString().equals("删除")) {
				...
				// 执行结算操作
				Toast.makeText(getActivity(), "暂时无法结算", Toast.LENGTH_SHORT)
						.show();
			}
